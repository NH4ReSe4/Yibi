...
- 手动记账与普通话语音记账
- 语音识别优先使用设备端模型，不可用时可由 Android 系统语音服务联网完成
...
账目、预算和周期模板仅保存在手机本地。应用不会上传账目，也不会保存原始录音；语音转写是否联网由手机上的系统语音服务决定。
...
创建以 `v` 开头的标签会触发 GitHub Actions 发布流程，例如 `v1.1`。仓库需要配置以下 Actions Secrets：
